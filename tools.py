...
- Referring patients to specialists via email
"""
...
load_dotenv()
# Referral email imports
from referral_email import fetch_all_doctors, find_relevant_doctor, send_referral_email
    ...
    for patient in state.patient_memory_snapshot.values():
        if patient["patient_name"].lower() == patient_name.lower():
            try:
                ...
                logger.error(f"Error generating summary for {patient_name}: {e}", exc_info=True)
                return f"Error generating summary: {e}"

    return f"No records found for patient '{patient_name}'."


async def refer_patient_to_specialist(_: Any, state: State) -> str:
    """Tool to find a relevant doctor and send a referral email for the selected patient."""
    patient_name = state.patient_name
    if not patient_name:
        return "No patient selected for referral."

    # Find the patient data
    for patient_id, patient in state.patient_memory_snapshot.items():
        if patient["patient_name"].lower() == patient_name.lower():
            try:
                # Fetch doctors and find a match
                doctors = fetch_all_doctors()
                matched_doctor = find_relevant_doctor(patient, doctors)

                if not matched_doctor:
                    return f"No suitable specialist found for {patient_name}."

                # Send referral email
                result = send_referral_email(patient, matched_doctor)
                return (
                    f"Referral sent to Dr. {matched_doctor['name']} "
                    f"({matched_doctor['specialization']}). {result}"
                )

            except Exception as e:
                logger.error(f"Error during referral for {patient_name}: {e}", exc_info=True)
                return f"Error processing referral: {e}"
...
TOOLS: List[Callable[..., Any]] = [
    ...
    refer_patient_to_specialist,  # <-- New referral tool added
]
